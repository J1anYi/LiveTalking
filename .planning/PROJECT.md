...
**Goal:** 修复优雅退出问题并添加 RAG 模式切换功能

**Target features:**
- 修复 Ctrl+C 后进程无法正常退出的问题（后台线程仍运行，端口被占用）
- Dashboard 添加 RAG 模式切换按钮（RAG-only vs RAG+模型知识）

**Key context:**
- 问题 1：asyncio 信号处理不完整，后台线程未正确关闭
- 问题 2：当前 LLM 只使用 RAG 上下文，需要支持混合模式
- 涉及文件：app.py（信号处理）、llm.py（prompt 构建）、dashboard.html（前端）
